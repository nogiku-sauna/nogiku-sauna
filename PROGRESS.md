...
### 🔴 まだ残っている作業（最優先・次のチャットで真っ先に）

**① server.jsの修正はまだVPSに反映されていません。** GitHubへのコミットは済んでいるが、`git pull`＋再起動はまだ実行していない。
つまり**コマンドインジェクションの脆弱性は、本番サーバーではまだ直っていない状態**（GitHub上のコードは直っている）。
しんさんにシリアルコンソールで、下記を**1行ずつ**実行してもらうこと（新しいADMIN_KEYの追加もこの中に含む）：
...
⚠️ `git pull`と`echo "ADMIN_KEY=..."`は**必ずセットで**実行すること。`.env`に`ADMIN_KEY`を追加する前に再起動すると、`/notifications`・`/failures`・`/dashboard`が一時的に誰の鍵でも開けなくなる（新旧どちらの鍵も一致しなくなるため）。

**② Google Apps Script「NOGIKUトラブル見張り番」の更新もまだ済んでいない。** 前回チャット終了時点で、**どのGoogleアカウント／どのプロジェクトに実際の「見張り番」があるのか特定できずに終わった**：
- ブラウザで開いていたGoogleアカウントは `sinriku24@gmail.com`（しんさんの個人アカウント）だったが、PROGRESS.mdには「`nogikusauna@gmail.com`（お店のアカウント）で作成済み」と記載されている
- `sinriku24@gmail.com`のApps Scriptプロジェクト一覧（3件）を確認したが、以前確認したはずの更新済み内容（api.nogikusauna.comへの書き換え）が見当たらなかった（古いURLのままの空プロジェクトのみ）
- **次のチャットで最初にしんさんに確認すること**：「見張り番」を確認・実行したときは、実際どちらのGoogleアカウントで開いていましたか？　それが分かったら、そのアカウント・プロジェクトで以下2点を直す：
...
**③ B・C・E・Fの残りの修正も未着手。** 直す方法はチャットで提示・確認済みなので、そのまま実装してよい：
- privacy.htmlの「ひな形です」文言削除
- `reservation_loss_estimate.html`を非公開化（リポジトリから削除）
- `pricing.html`・`access.html`・`faq.html`・`contact.html`をindex.htmlへ自動転送に変更
- `reservation_concierge.html`を削除（ユーザー確認済み）
- terms.html等に「雨天対応・ペット不可・喫煙ルール」を追記（貴重品は追記不要、ユーザー確認済み）
- privacy.htmlのSquareへのデータ提供の文言を業務委託の趣旨に調整

---

## 15. 次にやること（おすすめ順）

1. 🔴 上記「14章 残っている作業」の①②③（server.jsのVPS反映／Apps Script更新／サイト監査の残り修正）
2. チェックリストの実行（社員さんと一緒に。ユーザーが一人のため保留中）
3. データのリセット（テスト用データを公開前にゼロに戻す）
4. 旧Squareサイトの切り替え（オーバーブッキング防止のため）
